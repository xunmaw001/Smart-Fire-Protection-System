<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="处理结果" prop="chulijieguo">
          <el-select v-model="ruleForm.chulijieguo" placeholder="请选择处理结果">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="处理结果" prop="chulijieguo">
	      <el-input v-model="ruleForm.chulijieguo"
                placeholder="处理结果" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="处理时间" prop="chulishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.chulishijian" 
                type="datetime"
                placeholder="处理时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.chulishijian" label="处理时间" prop="chulishijian">
              <el-input v-model="ruleForm.chulishijian" 
                placeholder="处理时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `baoxiujieguo/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.baoxiujieguoCrossAddOrUpdateFlag = false;
                  this.parent.search();
